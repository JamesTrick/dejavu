...
class OrderValidator(ABC):
    @abstractmethod
    def validate(self, order: Order, fill_price: float, portfolio: Portfolio) -> tuple[bool, str | None]:
        """
...
class ShortValidator(OrderValidator):
    ...
    def validate(self, order: Order, fill_price: float, portfolio: Portfolio) -> tuple[bool, str | None]:
        if order.quantity < 0 and not self.allow_short:
            ...
            if not is_closing:
                return False, f"Short selling not permitted for {order.instrument.symbol}"
        return True, None
